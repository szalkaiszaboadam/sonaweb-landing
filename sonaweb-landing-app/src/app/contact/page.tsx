...
export default function StartPage() {
  ...
  return (
    <main className="relative flex h-[100dvh] w-full flex-col justify-between overflow-hidden bg-[#0A0A0A] text-[#F4F2F0]">
      
      {/* ── FELSŐ FOLYAMATJELZŐ CSÍK ── */}
      <div className="absolute left-0 top-0 z-50 h-1 w-full bg-[#1A1A1A]">
        <motion.div
      ...
      {/* ── KÖZÉPSŐ INTERAKTÍV TARTALOM (Viewport magassághoz igazítva) ── */}
      <div className="mx-auto flex w-full max-w-[640px] flex-1 flex-col justify-center px-6 pt-20 pb-10">
        
        {!isSuccessStep ? (
          <div className="flex flex-col">
            
            {/* Lépés navigáció és számláló */}
            <div className="mb-4 flex h-6 items-center justify-between">
              {step > 0 ? (
                <button
                  onClick={() => setStep(prev => prev - 1)}
                  className="group flex items-center gap-1.5 font-inter text-xs font-semibold uppercase tracking-wider text-[#5A5755] transition-colors hover:text-[#F4F2F0]"
                >
              ...
              <span className="font-inter text-xs font-bold uppercase tracking-widest text-[#BF2234]">
                {step + 1} / {activeQuestions.length + 1} Lépés
              </span>
            </div>

            {/* Kérdés címe */}
            <h1 className="mb-6 font-display text-[clamp(1.5rem,3.5vw,2.25rem)] font-extrabold uppercase leading-[1.1] tracking-tight text-[#F4F2F0]">
                ...
                >
                  {currentQuestion.options.map((option) => {
                    ...
                    return (
                      ...
                      >
                        <span className={`font-inter text-[14px] md:text-[15px] font-medium transition-colors ${
          ...
          >
            ...
            <h1 className="mb-2 font-display text-3xl font-black uppercase tracking-tight text-[#F4F2F0]">
              Kalkuláció elkészült
            </h1>
            
            <p className="mb-6 font-inter text-[14px] leading-relaxed text-[#9E9A98]">
              A részletes bontást elküldtük a(z) <span className="font-semibold text-[#F4F2F0]">{contactData.email}</span> címre.
            </p>

            <div className="flex w-full flex-col gap-4 rounded-2xl border border-[#1C1C1C] bg-[#111111] p-6 text-left">
      ...
      {/* ── KÉRÉSRE SZABOTT KÖTELEZŐ MIKRO-LÁBLÉC ── */}
      <footer className="w-full border-t border-[#141414] py-4 px-6">
        <div className="mx-auto flex max-w-[1340px] flex-col items-center justify-between gap-3 text-[11px] font-medium text-[#5A5755] sm:flex-row">
